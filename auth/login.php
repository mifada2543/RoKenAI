...
<body class="font-jakarta min-h-screen flex items-center justify-center overflow-hidden antialiased">
    ...
    <script>
        ...
        function applyTheme(t) {
            ...
            if (t === 'light') {
                ...
                document.querySelector('.auth-card, [class*="rounded-\\[28px\\]"]').style.setProperty('--bg-card', '#FFFFFF');
                document.querySelector('.auth-card, [class*="rounded-\\[28px\\]"]').style.setProperty('--border-color', 'rgba(0,0,0,0.08)');
            } else {
                html.setAttribute('data-theme', 'dark');
                icon.setAttribute('data-lucide', 'moon');
                document.querySelector('meta[name="theme-color"]')?.setAttribute('content', '#0B0F19');
                document.querySelector('.auth-card, [class*="rounded-\\[28px\\]"]').style.setProperty('--bg-card', '#1E293B');
                document.querySelector('.auth-card, [class*="rounded-\\[28px\\]"]').style.setProperty('--border-color', 'rgba(255,255,255,0.08)');
            }
